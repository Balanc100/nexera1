...
const App: React.FC = () => {
  ...
  const addToCart = (product: Product) => {
    setCart(prev => {
      ...
      if (existing) {
        // Removed stock limit check to allow backorders
      ...
      <nav className="bg-white border-b border-slate-200 sticky top-0 z-30 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
          <div className="flex items-center gap-2 text-emerald-600 cursor-pointer" onClick={() => setView('shop')}>
            ...
            <h1 className="text-xl font-bold tracking-tight text-slate-800 hidden sm:block">Nexera<span className="text-emerald-600">Order</span></h1>
          </div>
                      ...
                      return (
                        <div key={product.id} className="flex gap-4 p-3 rounded-xl hover:bg-slate-50 transition-colors border border-transparent hover:border-slate-200 group">
                          <div className="relative">
                            ...
                            />
                            {/* Removed the stock=0 overlay to allow backordering */}
                          </div>
                          <div className="flex-1 flex flex-col justify-between py-0.5">
                            <div>
                              ...
                              <div className="flex flex-wrap items-center gap-2 text-xs text-slate-500">
                                ...
                                <span className={remainingStock < 0 ? "text-red-500 font-bold" : "text-slate-500"}>
                              ...
                              {inCart ? (
                                <div className="flex items-center gap-3 bg-white shadow border border-slate-200 rounded-lg px-1.5 py-1">
                                  ...
                                  <button 
                                    ...
                                    // Removed disabled check for stock limit
                                  >
                                ...
                                <button 
                                  ...
                                  // Removed disabled={product.stock === 0} to allow backorders
                                  className="bg-slate-900 text-white px-4 py-2 rounded-lg text-xs font-bold hover:bg-slate-700 transition-all active:scale-95"
                                >
                                  เพิ่ม
                                </button>
                              )}
